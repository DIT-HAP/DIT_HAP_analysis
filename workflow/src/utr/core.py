"""
UTR Insertion Classification — Core Logic
==========================================

Shared constants, loaders, and classification functions for the UTR insertion
stage. Ported from
DIT_HAP_pipeline/workflow/notebooks/upstream_and_downstream_analysis.ipynb and
factored out of the original single-script port so the stage can be split into
independent Snakemake rules (prepare -> classify), each re-runnable on its own.
...
gene DR) and A_ratio (insertion A / gene A).

The downstream human-review / plotting notebook lives at
notebooks/domain_analysis/review_utr_insertions.ipynb (Task 9) — this module
