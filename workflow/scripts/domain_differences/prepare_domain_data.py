...
"""
Prepare Domain-Difference Data
===============================

Stage 1 of the domain-differences split: load the gene-level fitting results
and the insertion-level annotations reindexed onto the fitting_results
insertion universe (see workflow/src/domain_differences/core.py for the
duplicate-collapsing / reindexing details), then write both as parquet
intermediates consumed by the stats-computation rule:

- gene_result.parquet: gene-level fitting results, with legacy um/lam metric
  columns normalized to DR/DL.
- annotations.parquet: insertion-level annotations, deduped (any Feature
  passing IN_GENE_FILTER wins) and reindexed onto fitting_results' insertion
  universe.
...
class PrepareConfig:
    """Inputs and parquet outputs for the domain-difference data preparation."""
    ...
    def validate(self) -> None:
        ...
        for out in [self.output_gene_result, self.output_annotations]:
...
def run(config: PrepareConfig) -> None:
    ...
    logger.success(
        f"Prepared domain-difference data: {len(gene_result):,} genes, "
        f"{len(annotations):,} reindexed insertions"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Prepare domain-difference parquet intermediates")
