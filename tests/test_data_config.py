...
def test_list_dataset_names_includes_all_nine_projects():
    """Registry lists all registered projects under DIT_HAP_snakemake/projects/."""
    names = list_dataset_names()
    assert set(names) == {
        "LD_DIT_HAP", "HD_DIT_HAP",
...
def test_default_dataset_name():
    ...
    assert default_dataset_name() == "HD_DIT_HAP"
...
@pytest.mark.parametrize("name", ["Spikein", "Spore2YES6_1328", "LD_DIT_HAP", "LD_haploid"])
...
def test_ld_dit_hap_has_imputation():
    """LD_DIT_HAP has use_DEseq2_for_biological_replicates=True in its upstream config."""
    cfg = load_dataset_config("LD_DIT_HAP")
...
def test_reference_pombase_version():
    ...
    assert reference_pombase_version() == "2026-06-01"
