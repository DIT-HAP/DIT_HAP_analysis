...
def test_scale_features_caps_um_and_divides_lam():
    """um above 1.3 is clamped to 1.3; lam is divided by 10 (byte-faithful quirk #1)."""
    df = pd.DataFrame(
        {"um": [0.5, 1.3, 2.0, 1.29], "lam": [10.0, 20.0, 5.0, 100.0]},
        index=["g1", "g2", "g3", "g4"],
    )
    scaled = scale_features(df, ["um", "lam"])
    # um: 0.5 stays, 1.3 stays (strict < so equal is unchanged), 2.0 -> 1.3, 1.29 stays.
    assert list(scaled["um"]) == [0.5, 1.3, UM_CAP, 1.29]
    # lam divided by 10.
    assert list(scaled["lam"]) == [1.0, 2.0, 0.5, 10.0]


def test_scale_features_drops_nan_rows():
    """dropna defines the clustered set — genes with NaN um/lam are excluded."""
    df = pd.DataFrame(
        {"um": [0.5, np.nan, 0.8], "lam": [10.0, 20.0, np.nan]},
        index=["g1", "g2", "g3"],
    )
    scaled = scale_features(df, ["um", "lam"])
...
def test_perform_clustering_analysis_returns_four_methods_0based():
    ...
    data = pd.DataFrame(rng.random((60, 2)), columns=["um", "lam"])
...
def test_kmeans_is_deterministic_given_seed():
    ...
    data = pd.DataFrame(rng.random((80, 2)), columns=["um", "lam"])
